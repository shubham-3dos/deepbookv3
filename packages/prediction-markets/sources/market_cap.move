...
public(package) fun new_threshold(predict_id: ID, oracle_id: ID, ctx: &mut TxContext): MarketCap {
    MarketCap { id: object::new(ctx), predict_id, oracle_id, market_type: MARKET_TYPE_THRESHOLD, num_outcomes: 2 }
}

public(package) fun new_categorical(predict_id: ID, oracle_id: ID, num_outcomes: u8, ctx: &mut TxContext): MarketCap {
    MarketCap { id: object::new(ctx), predict_id, oracle_id, market_type: MARKET_TYPE_CATEGORICAL, num_outcomes }
}
...
#[test_only]
public fun create_test_market_cap_threshold(predict_id: ID, oracle_id: ID, ctx: &mut TxContext): MarketCap {
    new_threshold(predict_id, oracle_id, ctx)
}

#[test_only]
public fun create_test_market_cap_categorical(predict_id: ID, oracle_id: ID, num_outcomes: u8, ctx: &mut TxContext): MarketCap {
