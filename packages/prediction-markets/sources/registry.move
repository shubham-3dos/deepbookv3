...
public struct PriceOracleCreated has copy, drop, store {
    oracle_id: ID, oracle_cap_id: ID, expiry: u64,
    threshold: u64, threshold_above: bool, is_touch_market: bool,
}
public struct CategoricalOracleCreated has copy, drop, store {
    oracle_id: ID, oracle_cap_id: ID, expiry: u64, num_outcomes: u8,
}
...
public fun create_predict<Quote>(
    registry: &mut Registry, _admin_cap: &AdminCap, ctx: &mut TxContext,
): ID {
...
public fun admin_deposit<Quote>(
    predict: &mut predict::Predict<Quote>, _admin_cap: &AdminCap, coin: Coin<Quote>,
) {
    predict.deposit(coin);
}

public fun set_paused<Quote>(
    predict: &mut predict::Predict<Quote>, _admin_cap: &AdminCap, paused: bool,
) {
...
public fun create_price_oracle<Underlying>(
    _admin_cap: &AdminCap, cap: &OracleCapPrice, expiry: u64, threshold: u64,
    threshold_above: bool, is_touch_market: bool, touch_confirmations_required: u64,
    max_fair_price_delta: u64, ctx: &mut TxContext,
): ID {
    let oracle_id = oracle_price::create_oracle<Underlying>(
        cap, expiry, threshold, threshold_above, is_touch_market,
        touch_confirmations_required, max_fair_price_delta, ctx,
    );
    event::emit(PriceOracleCreated {
        oracle_id, oracle_cap_id: object::id(cap), expiry,
        threshold, threshold_above, is_touch_market,
    });
    oracle_id
}
...
public fun create_oracle_cap_categorical(_admin_cap: &AdminCap, ctx: &mut TxContext): OracleCapCategorical {
    oracle_categorical::create_oracle_cap(ctx)
}

public fun create_categorical_oracle(
    _admin_cap: &AdminCap, cap: &OracleCapCategorical,
    expiry: u64, num_outcomes: u8, ctx: &mut TxContext,
): ID {
    let oracle_id = oracle_categorical::create_oracle(cap, expiry, num_outcomes, ctx);
    event::emit(CategoricalOracleCreated {
        oracle_id, oracle_cap_id: object::id(cap), expiry, num_outcomes,
    });
    oracle_id
}

public fun resolve_categorical_oracle(
    oracle: &mut oracle_categorical::OracleCategorical, _admin_cap: &AdminCap,
    cap: &OracleCapCategorical, winning_outcome: u8, clock: &sui::clock::Clock,
) {
...
public fun register_threshold_market<Quote>(
    _admin_cap: &AdminCap, predict: &predict::Predict<Quote>,
    oracle_id: ID, ctx: &mut TxContext,
...
public fun register_categorical_market<Quote>(
    _admin_cap: &AdminCap, predict: &predict::Predict<Quote>,
    oracle_id: ID, num_outcomes: u8, ctx: &mut TxContext,
): market_cap::MarketCap {
...
#[test_only]
public fun create_admin_cap_for_testing(ctx: &mut TxContext): AdminCap { AdminCap { id: object::new(ctx) } }
